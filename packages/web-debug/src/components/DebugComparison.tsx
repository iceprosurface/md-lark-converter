...
interface DebugComparisonProps {
  ourData: any;
  larkData: any;
}

function DebugComparison({ ourData, larkData }: DebugComparisonProps) {
  if (!ourData || !larkData) {
    return (
            ...
            <div key={index} className="flex items-start gap-2">
              ...
              <div className="flex-1">
                ...
                <div className="flex gap-4">
                  {item.ours && (
                    <div className="flex-1">
                      <div className="text-blue-300 text-xs mb-1">我们:</div>
                      <div className="text-white/70 text-xs whitespace-pre-wrap break-all bg-black/40 rounded p-2">
                        {item.ours}
                      </div>
                    </div>
                  )}
                  {item.lark && (
                    <div className="flex-1">
                      <div className="text-yellow-300 text-xs mb-1">飞书:</div>
                      <div className="text-white/70 text-xs whitespace-pre-wrap break-all bg-black/40 rounded p-2">
                        {item.lark}
                      </div>
                    </div>
      ...
      <div className="bg-black/30 border border-white/10 rounded-xl p-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <div className="text-xs text-white/70 whitespace-pre-wrap break-all max-h-60 overflow-auto bg-black/40 rounded p-2">
...
function compareData(ours: any, lark: any): ComparisonItem[] {
  ...
  const ourIsvBlocks = Object.values(ours.recordMap).filter((r: any) => r.snapshot.type === 'isv');
  const larkIsvBlocks = Object.values(lark.recordMap).filter((r: any) => r.snapshot.type === 'isv');
  ...
  if (ourIsvBlocks.length > 0) {
    const ourFirstIsv = ourIsvBlocks[0] as any;
    const larkFirstIsv = larkIsvBlocks[0] as any;
...
function generateComparisonHtml(data: any): string {
  ...
  for (const recordId of data.recordIds.slice(0, 5)) {
...
function generateSampleLarkHtml(data: any): string {
  const jsonStr = JSON.stringify(data);
  const encodedJson = jsonStr
    .replace(/&/g, '&amp;')
    .replace(/"/g, '&quot;')
